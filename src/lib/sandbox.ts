...
function cleanCodeForExecution(code: string): string {
  ...
  cleaned = cleaned.replace(/(const\s+\w+\s*:\s*\([^)]*\)\s*=>)\s*<[A-Z]\w*(?:,\s*[A-Z]\w*)*>/g, '$1')
  ...
  // Supprimer les types dans les fonctions fléchées typées
  // Exemple: const test: (x: number) => number = (x) => ... -> const test = (x) => ...
  cleaned = cleaned.replace(/(const|let|var)\s+(\w+)\s*:\s*\([^)]*\)\s*=>[^=]+=/g, '$1 $2 =')

  // Supprimer les "as const", "as number", etc. (assertions de type)
  cleaned = cleaned.replace(/\s+as\s+(const|number|string|boolean|any|unknown|void|never|readonly|readonly\s+\[[^\]]+\])/g, '')
...
export function executeCode(code: string): SandboxResult {
  ...
  try {
    ...
    if (!cleanCode.trim()) {
      throw new Error('Le code est vide après nettoyage. Vérifiez que le fichier contient du code exécutable.')
    }
